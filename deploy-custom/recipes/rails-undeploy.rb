...
node[:deploy].each do |application, deploy|
  if deploy[:application_type] != 'rails'
    Chef::Log.debug("Skipping deploy-custom::rails-undeploy application #{application} as it is not an Rails app")
    next
  end
  ...
  when 'nginx_rainbows'
    ...
    execute 'stop rainbows and restart nginx' do
      command "sleep #{deploy[:sleep_before_restart]} && \
             /srv/www/#{application}/shared/scripts/rainbows stop"
      notifies :restart, "service[nginx]"
      action :run
    end
